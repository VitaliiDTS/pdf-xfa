...
/**
 * Step 3 — Acrobat COM automation barcode rendering.
 *
 * Flow:
 *   1. Fill XFA form via importXFAFormsData() and save → filled_result.pdf
 *   2. Invoke scripts/acrobat_trigger_barcodes.ps1 via PowerShell:
 *      - Opens filled_result.pdf in Acrobat Pro through COM (AcroExch.PDDoc)
 *      - Calls BC.showBarcodes() + BC.encodeAll() in the XFA JavaScript engine
 *      - Saves to barcoded_result.pdf
 *   3. Re-open barcoded_result.pdf with APDFL
 *   4. flattenXFAFormFieldsAsIfPrinted() → final_flattened.pdf
 *   5. inspectPageResources() to confirm Image XObjects (rendered PDF417 barcodes)
 *
 * Output: output/final_flattened.pdf
 */
public class AcrobatBarcodeStep {

    private static final String FILLED_PDF   = XfaFormFiller.BASE_DIR + "/output/acrobat_filled.pdf";
    private static final String BARCODED_PDF = XfaFormFiller.BASE_DIR + "/output/acrobat_barcoded.pdf";
    private static final String FINAL_PDF    = XfaFormFiller.BASE_DIR + "/output/acrobat_final_flattened.pdf";
    private static final String PS_SCRIPT    = XfaFormFiller.BASE_DIR + "/scripts/acrobat_trigger_barcodes.ps1";
    ...
    public static void run() throws Exception {
        ...
        System.out.println("[INFO] AcrobatBarcodeStep ready.  APDFL: " + Library.getAPDFLVersion());
        ...
        try {
            ...
            // ── 2. Kill any leftover Acrobat processes (may hold output files) ──
            killAcrobat();
            ...
            // ── 3. Acrobat COM step: trigger XFA JS barcode rendering ─────────
            System.out.println("\n[STEP 2/3] Acrobat COM → trigger BC.encodeAll() → " + BARCODED_PDF);
            boolean psOk = runPowerShellScript(PS_SCRIPT, FILLED_PDF, BARCODED_PDF);
            if (!psOk || !new File(BARCODED_PDF).exists()) {
                System.err.println("[ERROR] Acrobat step failed or output not produced.");
                System.err.println("        Check that Adobe Acrobat Pro is installed and accessible via COM.");
                return;
            }
            long barcodedSize = new File(BARCODED_PDF).length();
            System.out.printf("[OK]  Barcoded PDF: %.1f KB%n", barcodedSize / 1024.0);
            System.out.printf("      Size change: %+.1f KB (positive = barcode data added)%n",
                    (barcodedSize - filledSize) / 1024.0);

            // ── 4. Re-open with APDFL and flatten ─────────────────────────────
    ...
    /**
     * Invokes the PowerShell script with -ExecutionPolicy Bypass so it runs
     * without requiring a signed script policy.
     *
    ...
    private static boolean runPowerShellScript(String scriptPath, String inputPdf, String outputPdf)
        ...
        ProcessBuilder pb = new ProcessBuilder(List.of(
                ...
                "-File", toWinPath(scriptPath),
                "-InputPdf",  toWinPath(inputPdf),
                "-OutputPdf", toWinPath(outputPdf)
        ));
        pb.redirectErrorStream(true);   // merge stderr into stdout
        pb.environment().put("PSModulePath", ""); // avoid slow module scan
    ...
    /** Convert forward-slash path to Windows backslash path. */
    private static String toWinPath(String path) {
        return new File(path).getAbsolutePath();
    }

    /** Kill any running Acrobat process so it doesn't hold output files locked. */
    private static void killAcrobat() {
        try {
            ...
            // non-fatal — Acrobat may simply not be running
        }
    }

    /** Locate powershell.exe or pwsh.exe on Windows. */
